...
from transformer_lens.utilities.activation_functions import ActivationFunction, ModifiedAct
...
class CanBeUsedAsMLP(nn.Module):
    ...
    def select_activation_function(self) -> None:
        ...
        if self.cfg.use_lrp and 'Identity-rule' in self.cfg.LRP_rules:
            self.act_fn = ModifiedAct(ActivationFunctionFactory.pick_activation_function(self.cfg), transform='identity')
        else:
